...
const Select = forwardRef<any, ISelectProps>(({ placeholder, placeholderStyles, items, renderItem, ...props }, ref) => {
  return (
    <StyledTextField
...
export const LabelledSelect: FC<LabelledSelectProps> = ({ id, label, showError, error, placeholder, items, renderItem, ...props }) => {
  return (
    <>
...
export const ControlledSelect: FC<ControlledSelectProps> = ({ placeholder, name, rules, defaultValue, items, renderItem, ...props }) => {
  const { field: { onChange, onBlur, value, ref } } = useController({ name, rules, defaultValue: defaultValue ?? placeholder })
